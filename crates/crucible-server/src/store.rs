...
impl Store {
    ...
    pub fn save_match(
        ...
        conn.execute(
            "INSERT INTO matches (map_seed, p1_type, p2_type, result, duration_ticks, replay, created_at)
    ...
    pub fn list_matches(&self, limit: u32) -> Result<Vec<StoredMatch>, rusqlite::Error> {
        ...
        let mut stmt = conn.prepare(
            "SELECT id, map_seed, p1_type, p2_type, result, duration_ticks, created_at
...
mod tests {
    ...
    fn stored_match_replays_byte_identically() {
        ...
        use crucible_sim::{serialize, Game, GameConfig, Map, Player, Replay, ReplayResult};

        let store = Store::in_memory().unwrap();
        let cfg = GameConfig {
            timeout_ticks: 400, // short match for the test
            ..GameConfig::default()
        };
        let seed = 99u64;
        ...
        while !g.is_over() {
            if g.is_command_tick() {
                ...
                for c in &cmds {
                    replay.record(g.tick, Player::P1, c.clone());
                }
        ...
        let mut repro = serialize::replay_to_game(&loaded);
        while repro.tick < g.tick && !repro.is_over() {
            repro.step();
        }
        assert_eq!(
