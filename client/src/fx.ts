...
export interface DockingEffect {
  harvesterId: number;
  hx: number;
  hy: number;
  rx: number;
...
export class FXEngine {
  ...
  deaths: DeathEffect[] = [];
  miningEffects = new Map<number, MiningEffect>();
  dockingEffects = new Map<number, DockingEffect>();
  ...
  update(dt: number): void {
    ...
    // 6. Update mining lasers
    for (const [id, m] of this.miningEffects) {
      m.life -= dt;
      if (m.life <= 0) this.miningEffects.delete(id);
    }

    // 7. Update docking beams
    for (const [id, d] of this.dockingEffects) {
      d.life -= dt;
      if (d.life <= 0) this.dockingEffects.delete(id);
    }
  ...
  recordVehicleMovement(id: number, kind: string, x: number, y: number, angle: number): void {
    ...
    if (dist >= 0.12) {
      const isCrawler = kind === "Artillery" || kind === "Harvester";
  ...
  /** Set active mining laser */
  setMining(harvesterId: number, hx: number, hy: number, ox: number, oy: number): void {
    this.miningEffects.set(harvesterId, {
      harvesterId,
      hx,
      hy,
      ox,
  ...
  /** Set active refinery docking transfer */
  setDocking(harvesterId: number, hx: number, hy: number, rx: number, ry: number): void {
    this.dockingEffects.set(harvesterId, {
      harvesterId,
      hx,
      hy,
      rx,
  ...
  drawAirLayer(
    ...
    // 1. Mining Beams
    for (const m of this.miningEffects.values()) {
      const sx = cam.screenX(m.hx);
      const sy = cam.screenY(m.hy);
      const tox = cam.screenX(m.ox);
      const toy = cam.screenY(m.oy);
    ...
    // 2. Docking Ore Conveyor Beams
    for (const d of this.dockingEffects.values()) {
      const sx = cam.screenX(d.hx);
      const sy = cam.screenY(d.hy);
      const tox = cam.screenX(d.rx);
      const toy = cam.screenY(d.ry);
    ...
    // 4. Realistic Military Explosions (Tight, intense fireball & black smoke clouds)
    ...
    // 5. Particles (Shrapnel sparks & rising soot)
